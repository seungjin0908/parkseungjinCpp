...
namespace DBConsole
{
    ...
    {
        public async Task<ResDtoGetPlayerList> GetplayerList(ReqDtoGetPlayerList request)
        {
            // 유니티에서는 사용하면 안된다.
            ...
            // localhost : 내 PC 고유 값
            // www.naver.com
            // PORT : https (443)가 아닐시, 기입
            var respones = await httpClient.GetAsync("");

            var rv= await respones.Content.ReadAsStringAsync();

            return rv ?? new ResDtoGetPlayerList();
        }

        public async Task<ResDtoGetPlayerList> GetplayerList(ResDtoGetInventory request)
        {
            // 유니티에서는 사용하면 안된다.
            HttpClient httpClient = new HttpClient();
            ...
            return rv ?? new ResDtoGetInventory();
        }
    }
