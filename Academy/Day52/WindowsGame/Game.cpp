...
void Game::Render()
{
	...
	// 모두 Windows API를 활용해서 그림을 그리고 있었습니다.
			...
			
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					hr = pDWriteFactory->CreateTextFormat(
						...
						DWRITE_FONT_STYLE_NORMAL,		// 스타일 (italic 폰트로 쓸지등등)
						DWRITE_FONT_STRETCH_NORMAL,		// Strech 유무
						20.0f,							// 폰트크기
						L"en-us",
						&pTextFormat
					);
				...
				pRenderTarget->DrawText(
					timeStr.c_str(),							//문자열
					static_cast<uint32>(timeStr.length()),		//문자열길이
					pTextFormat,								//텍스트포맷
					layoutRect,
					pBrush
				);
