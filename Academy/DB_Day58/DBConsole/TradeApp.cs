...
namespace DBConsole
{
    ...
    public class TradeApp
    {
        ...
        private void SellScene()
        {
            ...
            // 1. 실제로 가지고있는지 체크
            var playerItemEntity = itemList.Where(Item => Item.Id == selected).FirstOrDefault();
            if(false == itemList.Any(item => item.Id == selected))
            {
        ...
        private void BuyScene()
        {
            ...
            // 1. 무엇을 구매할건지 Id 받기 (TblTradeItem의 Id)
            input = Console.ReadLine();
            selected = int.Parse(input ?? "");

            // 2. 돈이 있는지 체크
            var tradeItemEntity = Managers.Db.TblTradeItems.Where(playerItem => playerItem.Id ==
            ...
            tradeItemEntity.BuyerPlayerId = Managers.List;
            Managers.Db.TblTradeItems.Remove(tradeItemEntity);
            //  - DB에서 나한테 아이템 하나 추가

            // 아이템이 존재한다면, 카운트 1증가 Update
            // 아이템을 첫득템이라면, Entity Add
            if(Managers.Db.TblPlayerItems.Add(playerItem=>playerItem.OwnerPlayerId== Managers.User.Id &&
            playerItem.ItemId==tradeItemEntity.ItemId))
            {
