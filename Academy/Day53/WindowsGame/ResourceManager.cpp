...
DXSprite* ResourceManager::CreateDXSprite(const wstring& key, const wstring& path)
{
	if (_resources.contains(key))
	{
		return GetDXSprite(key);
	}

	DXSprite* dxSprite = new DXSprite();

	DXSpriteInfo info;
	info.filePath = _resourcePath + path;
	dxSprite->SetInfo(info);

	_resources[key] = dxSprite;

	return dxSprite;
}
DXSprite* ResourceManager::GetDXSprite(const wstring& key)
{
	return dynamic_cast<DXSprite*>(_resources[key]);
}


DXFlipbook* ResourceManager::CreateDXFlipbook(const wstring& key, DXFlipbookInfo info)
{
	if (_resources.contains(key))
	{
		return GetDXFlipbook(key);
	}

	DXFlipbook* flipbook = new DXFlipbook();
	flipbook->SetInfo(info);
	_resources[key] = flipbook;
	return flipbook;
}
DXFlipbook* ResourceManager::GetDXFlipbook(const wstring& key)
{
	return dynamic_cast<DXFlipbook*>(_resources[key]);
}